...
import { Logout, Visibility, Close, Checklist } from '@mui/icons-material'
import { useNavigate } from 'react-router-dom'
import "./../../styles/dashboard.css"
import NavOrdre from './../../components/NavOrdre'
import vol from "./../../assets/vol.jpeg"



export default function ContAssi() {
  ...
  const [modalOdr, setModalOrd] = useState(false)
  ...
  const ordProject = () => {
    ...
    var raw = JSON.stringify({
      "bon_achat": projet.bon_achat,
      "bon_commande": projet.bon_commande,
      "num_service": projet.num_service,
      "num_accord": projet.num_accord,
      "date_travaux": projet.date_travaux
    });
    ...
    fetch("http://localhost:4000/projet/programme?numero="+projet.numero, requestOptions)
      .then(response => response.text())
      .then(result => {
        let a = JSON.parse(result)
        if (a.numero) {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:4000/projet/2", requestOptions)
      .then(response => response.text())
  ...
  return (
    <div className='dashboard'>
      ...
      {modalOdr && <div className="modal">

        <div className="modal-content">
          <Close className='close' onClick={() => setModalOrd(false)} />
          <div className="head">
            Complete Projet {projet.numero}
          </div>
          <div className="body">
            <div className="text-field">
              <span>Numéro d'ordre de service : </span>
              <span><input type="text" value={projet.num_service} onChange={(e) => setProjet({ ...projet, num_service: e.target.value })} /></span>
            </div>
            <div className="text-field">
              <span>Numéro d'accord : </span>
              <span><input type="text" value={projet.num_accord} onChange={(e) => setProjet({ ...projet, num_accord: e.target.value })} /></span>
            </div>
            <div className="text-field">
              <span>Num. de bon de commande : </span>
              <span><input type="text" value={projet.bon_commande} onChange={(e) => setProjet({ ...projet, bon_commande: e.target.value })} /></span>
            </div>
            <div className="text-field">
              <span>Numéro de bon d'achat : </span>
              <span><input type="text" value={projet.bon_achat} onChange={(e) => setProjet({ ...projet, bon_achat: e.target.value })} /></span>
            </div>
            <div className="text-field">
              <span>Date des travaux : </span>
              <span><input type="date" value={projet.date_travaux} onChange={(e) => setProjet({ ...projet, date_travaux: e.target.value })} /></span>
            </div>
          </div>
          <div className="btns">
            <span onClick={() => setModalOrd(false)}>Annuler</span>
            <span onClick={() => ordProject()}>Valider</span>
          </div>
        </div>
      ...
      <main>
        <header>
          <div className="left">
            Projets Assignés aux Techniciens
          </div>
          <div className="right">

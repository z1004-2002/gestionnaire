    ...
    fetch("http://localhost:4000/projet/create", requestOptions)
      ...
      .then(result => {
        ...
        if (a.numero) {
          ...
          fetch("http://localhost:4000/image?numero=" + a.numero + "&type=avant", requestOptions)
            .then(response => response.text())
            .then(result => {
              console.log(result)
    ...
    <div className='dashboard'>
      ...
      {modalAdd && <div className="modal">
        ...
        <div className="modal-content">
          ...
          <span className='et'>Photo Avant : <input type="file" placeholder='Ajouter des images'
            onChange={(e) => {
      ...
      {modalEdit && <div className="modal">
        <div className="modal-content">
          <Close className='close' onClick={() => setModalEdit(false)} />
          <div className="head">
            EDITER PROJET
          </div>
          <div className="body">
            <div className="text-field">
              <span>Ville : </span>
              <span><input type="text" value={editProjet.ville} onChange={(e) => setEditProjet({ ...editProjet, ville: e.target.value })} /></span>
            </div>
            <div className="text-field">
              <span>Quartier : </span>
              <span><input type="text" value={editProjet.quartier} onChange={(e) => setEditProjet({ ...editProjet, quartier: e.target.value })} /></span>
            </div>
            <div className="text-field">
              <span>Localisation GPS : </span>
              <span><input type="text" value={editProjet.gps} onChange={(e) => setEditProjet({ ...editProjet, gps: e.target.value })} /></span>
            </div>
            <div className="text-field">
              <span>Type : </span>
              <span>
                <select id="monselect" value={editProjet.type} onChange={(e) => setEditProjet({ ...editProjet, type: e.target.value })} >
                  <option value="T1">T1</option>
                  <option value="T2">T2</option>
                  <option value="T3">T3</option>
                  <option value="T4">T4</option>
            ...
            <div className="description">
              <p>Description : </p>
              <p><textarea name="description" value={editProjet.description} onChange={(e) => setEditProjet({ ...editProjet, description: e.target.value })} id="description" cols="54" rows="4"></textarea></p>
            </div>
          </div>
          <span className='et'>Photo Avant : <input type="file" placeholder='Ajouter des images' /></span>
          <div className="btns">
            <span onClick={() => setModalEdit(false)}>Annuler</span>
            <span onClick={() => editProject()}>Valider</span>
          </div>
        </div>
      </div>}
      ...
      <main>
        <header>
          <div className="left">
            Nouveaux Projets
            <span className='add' onClick={() => setModalAdd(true)}>
              Ajouter <Add className='icon' />
            </span>
          </div>
          <div className="right">
